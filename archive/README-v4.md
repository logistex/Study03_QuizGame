<!-- 2026-08-24 11:05 KST, 2차 계획서 검토 반영과 문서 규칙 정비까지 끝난 시점으로 보존. 1차 시점으로 쓰여 있던 마지막 판이다 -->
...
**4단계 이후 화면은 순위 기록을 공유한다.** 4단계에서 만든 기록이 5단계와 본편에도 보인다. 여섯 주소가 같은 오리진이기 때문이다. 1~3단계는 저장 함수 자체가 없어 기록을 만들지 못한다.

주소의 `-final`은 **검토 반영까지 끝난 시점**이라는 뜻이다. 2단계만 `-final`이 없는데, 2단계 검토를 3단계와 묶어 받아 단독 최종본이 존재하지 않기 때문이다.

## 폴더 구성

```
/            배포 산출물 네 개, README, CLAUDE.md
spec/        현재 유효한 규격 (PRD, 1차 구현 계획, 2차 구현 계획)
...
| 문서 | 내용 |
|---|---|
| [PRD](spec/PRD.md) | 요구사항 정본. 2차 개정으로 2차 기준으로 다시 썼다 |
| [1차 구현 계획](spec/IMPL-PLAN.md) | [`archive/PRD-v5.md`](archive/PRD-v5.md) 6절의 여섯 단계를 옮긴 계획 |
| [2차 구현 계획](spec/IMPL-PLAN-2.md) | PRD 6장의 다섯 단계를 옮긴 계획 |

**PRD는 하나이고 계획서는 둘이다.** 정본이 둘이 되면 어느 쪽이 맞는지 매번 판단하게 되므로 PRD는 나누지 않았다. 2차 착수 시점의 PRD는 [`archive/PRD-v6.md`](archive/PRD-v6.md)에 있다.
...
| [25 열린 항목](history/25-open-items.md) | 미리보기 배포, 사파리 낭독, 5단계로 넘긴 셋 |
| [26 4단계 재검토](history/26-stage4-review-round2.md) | 규격이 코드를 따라오지 못한 것 2건 |
| [27 5단계 작업 보고](history/27-stage5-report.md) | 반응형과 접근성, 에러 경로 점검. 코드 변경 없음 |
| [28 6단계 작업 보고](history/28-stage6-report.md) | 배포와 배포 후 확인, 단계별 스냅숏 구조 |
...
| [46 kh_14 3차 판정](history/46-kh14-round3.md) | 새 날짜 셋을 고종실록으로 확인. 권고 1건, 문항과 해설이 국호를 다르게 지목 |
| [47 kh_14 2차 대조](history/47-kh14-round2.md) | 사실 넷을 출처 둘씩. 필수와 권고 0건, 멈추는 선을 제안 |
| [48 kh_14 왕복을 닫는다](history/48-kh14-close.md) | 문항에서 낱말 하나를 빼고 닫음. 멈추는 선을 규격에 넣음 |
| [49 과학 추가 5문항 판정](history/49-sc-additions-review.md) | 사실 열둘을 출처 둘씩. 필수와 권고 0건, 선택 1건 |
| [50 과학 추가 5문항 검증 (2차)](history/50-sc-additions-review2.md) | 자동 검사 위반 0건. 필수와 권고 0건, 선택 1건 |
| [51 과학 추가 5문항 종결](history/51-sc-add-close.md) | 선택 2건을 고치지 않기로 정하고 닫음. 멈추는 선을 넣은 뒤 처음 돈 왕복 |
...
| [57 PRD 2차 개정 검토](history/impl_v2/57-prd-phase2-review.md) | 필수 6건, 권고 5건. 근거 문서가 같은 커밋에서 고쳐진 것과 계획서 참조가 끊긴 것 |
...
[1차 구현 계획](spec/IMPL-PLAN.md)은 [`archive/PRD-v5.md`](archive/PRD-v5.md) 6절의 여섯 단계를 그대로 골격으로 쓰고, 무거운 단계만 하위 작업으로 쪼갰다. 단계마다 만들 파일, 정의할 함수와 그 책임, 선행 조건, 그리고 **무엇을 눌러 무엇을 확인하는지**까지 적었다.

빌드 도구도 테스트 프레임워크도 쓰지 않으므로 검증 수단은 셋뿐이다. 페이지 로드 시점에 도는 데이터 검사 함수, 문항 규칙 여덟 가지를 보는 클로드의 판정, 그리고 브라우저에서 직접 밟는 수동 체크리스트다.
