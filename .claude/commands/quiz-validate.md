...
`spec/PRD.md` 3.6절 "규칙마다 무엇이 확인하는가" 표가 정한다. 아래 1장과 2장의 분류도 그 표에서 나왔다.

## 1. 자동으로 포착되는 것

확인 수단 표가 "데이터 검사(4.2절)"로 표시한 일곱 규칙이다. `script.js`의 `validateData()`가
...
난이도 배분과 정답 인덱스 분포는 판정이 아니라 세기다. 확인 수단 표가 그 자리를
`/quiz-stats`에 두었으므로 이 명령어는 세지 않는다. **두 명령어가 같은 것을 세면
어느 쪽이 낸 결과인지 알 수 없다.**
