...
확인 수단 표가 "계획서 1.8절 육안 검토"로 표시한 여덟 규칙이다. 검사 함수가 통과해도
...
판정은 사람이 하되, 계획서 1.8절이 **볼 곳을 좁히는 목록**을 함께 정해 두었다. 다섯째, 여섯째, 여덟째 줄이 그것이다.
...
- **자동 검사 결과와 사람의 판단을 구분하여 적는다.** 둘을 섞으면 무엇이 재현되는 판정인지 알 수 없다
